...
from Football_Event_Detector.YOLO.utility_functions import yolo_utils 
...
class Yolo_v1_loss(nn.Module):
    ...
    def forward(self, predictions, target):
        ...
        # Torch will infer reshape during training (-1)
        predictions = predictions.reshape(-1, self.split, self.split, self.classes + self.boxes * 5)
        
        iou_b1 = yolo_utils.intersection_over_union(predictions[..., 21:25], target[..., 21:25])
        iou_b2 = yolo_utils.intersection_over_union(predictions[..., 26:30], target[..., 21:25])
